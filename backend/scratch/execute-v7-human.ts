...
async function main() {
  console.log('================================================================');
  console.log('🚀 RELAY OUTREACH V7 — HUMAN ENGINEER EMAILS REGENERATION & AUDIT');
  console.log('================================================================\n');
  ...
  console.log(`Processing ${targets.length} distinct targets with RELAY OUTREACH V7 Rules...\n`);
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    console.log(`----------------------------------------------------------------`);
    console.log(`[Target ${i + 1}/${targets.length}] Generating V7 Email for: ${prospect.email} (${company.companyName}) [Style Index: ${i % 10}]`);
    ...
    const foundForbiddenPhrases: string[] = [];
    const lowerBody = savedDraft.body.toLowerCase();
    for (const fp of FORBIDDEN_PHRASES) {
      if (lowerBody.includes(fp)) {
        foundForbiddenPhrases.push(fp);
      }
    }

    const lastPara = paragraphs.length > 0 ? paragraphs[paragraphs.length - 1].trim() : '';
    const validEnding = ALLOWED_ENDINGS.includes(lastPara);
  ...
  console.log('\n================================================================');
  console.log('📊 RELAY OUTREACH V7 — AUDIT SUMMARY ACROSS ALL TARGETS');
  console.log('================================================================\n');

  let allPassed = true;
  for (const r of results) {
    const validWords = r.wordCount >= 55 && r.wordCount <= 90;
    const idealWords = r.wordCount >= 60 && r.wordCount <= 85;
    const validParas = r.paragraphCount === 4;
    const validSig = !r.hasSigOrClosing && r.validEnding;
    const validSubject = ALLOWED_SUBJECT_LIST.some((s) => r.subject.includes(s) || s === r.subject);
    const cleanPhrases = r.foundForbiddenPhrases.length === 0;
    const passed = validWords && validParas && validSig && validSubject && cleanPhrases && !r.requiresManualReview;
    if (!passed) allPassed = false;

    console.log(`[${r.company}] ${r.email}`);
    console.log(`  Subject:         ${r.subject} ${validSubject ? '✅' : '❌'}`);
    console.log(`  Word Count:      ${r.wordCount} words (Acceptable: 55-90, Preferred: 65-80) ${validWords ? (idealWords ? '✅ (Ideal)' : '✅') : '❌'}`);
    console.log(`  Paragraphs:      ${r.paragraphCount} ${validParas ? '✅' : '❌'}`);
    console.log(`  Valid Ending:    ${validSig ? '✅' : '❌'}`);
    console.log(`  Forbidden Words: ${cleanPhrases ? '0 (Clean) ✅' : '❌ ' + r.foundForbiddenPhrases.join(', ')}`);
  ...
  console.log(`OVERALL STATUS: ${allPassed ? '🎉 100% RELAY OUTREACH V7 COMPLIANT' : '⚠️ COMPLIANCE ISSUES FOUND'}`);
